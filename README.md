...
将个人剪藏文件夹（AI 主题 Markdown 文章）构建为**时尚简洁、可查阅、可分享**的静态知识网站。
按 **作者** 与 **标签** 两个维度组织，支持全文检索与浅/深色主题切换。

## 站点结构

- `index.html` — 总览页：顶栏搜索 + 侧栏（作者 / 标签筛选）+ 卡片网格，点击任一维度可下钻（URL 参数可分享）
- `articles/*.html` — 每篇文章独立全文页（含「查看原文」外链）
- `assets/data.json` — 文章元数据 + 正文纯文本（供前端检索）
...
本站通过 `fetch` 加载 `data.json`，需经 HTTP 服务（不能直接 `file://` 打开）：
...
- 文章：976 篇
- 作者：90 位
- 标签：1538 个（侧栏默认展示出现 ≥3 次的常用标签，可展开全部）
