...
function firstImage(md) {
  // 取文章首张远程图片作为封面（含微信图床）。微信防盗链靠页面 <meta name="referrer" content="no-referrer"> 绕过，
  // 无需把图片下载进仓库——保持仓库轻量且封面始终跟随源文件。
  const re = /!\[[^\]]*\]\((https?:\/\/[^)\s]+)\)/g;
...
console.log(`封面：取文章首图外链（微信图床靠 no-referrer meta 直接加载，无需入库）`);
